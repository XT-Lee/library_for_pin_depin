...
class static_points_analysis_2d:  # old_class_name: PointsAnalysis2D
    ...
    def get_coordination_number_conditional(self, lattice_constant=3, method="bond_length_method"):
        ...
        self.count_coordination = np.zeros((cn_max,))  # np.zeros([cn_max, 1])
  ...
  File "/home/tplab/xiaotian_file/lxt_code_py/control_table_multi_thread.py", line 13, in <module> (Current frame)
    ...
    def get_neighbor_cloud_method_voronoi(self, png_filename=None):  # checked right
        ...
        for k in self.edge_cut_positions_list[0]:  # range(num):
            ...
            plt.scatter(
                self.points[nbr, 0] - self.points[k, 0],
                self.points[nbr, 1] - self.points[k, 1],
                c='k', marker='.')
    ...
    def draw_bonds_conditional_bond(
            self, check=[0.9, 2.0],
            png_filename=None, nb_change=None, x_unit='(um)', show_traps=False,
    ...
    def draw_bonds_conditional_bond_oop(
            self, check, png_filename=None, xy_stable=None, nb_change=None, x_unit='(um)',
            LinearCompressionRatio=None, trap_filename=None, axis_limit=None):
        R"""
        ...
        if not trap_filename is None:
            self.draw_bonds.plot_traps(trap_filename=trap_filename,
                                       LinearCompressionRatio=LinearCompressionRatio)
        if not nb_change is None:
            self.draw_bonds.plot_neighbor_change(xy_stable, nb_change)
        if not png_filename is None:
            self.draw_bonds.save_figure(png_filename)

    def draw_bonds_conditional_bond_for_image_oop(
            self, image, check, png_filename=None, xy_stable=None, nb_change=None, x_unit='(um)',
            LinearCompressionRatio=None, trap_filename=None, axis_limit=None):
        R"""
        xy(self.points) must be a frame of txyz( not txyz_stable)!
        xy_stable: a frame of txyz_stable
        ...
        if not axis_limit is None:
            ...
            self.draw_bonds.restrict_axis_limitation(xlim, ylim)
        line = self.draw_bonds.draw_points_with_conditional_bond(
            self.points, self.bond_length, bond_length_limmit=check)
        # self.draw_bonds.draw_bonds_conditional_bond(self.points,self.bond_length,bond_length_limmit=check,x_unit=x_unit)
        if not trap_filename is None:
            self.draw_bonds.plot_traps(trap_filename=trap_filename,
    ...
    def print_basic_property(self):
        ...
        # when rcut=12.5, Uparticle = kBT = 1; rcut=15, Up=0.47
        energy_interaction = epsilon*np.exp(-kappa)
    ...
    def count_polygon_n(self, n_edges):
        ...
        while sum(all_chain_count) < tri_count_max:
            if lt_temp[i, 0] == -1:
                ...
                index1 = np.where((lt_temp[:, 0] == self.linked_triangles[i, 1]) | (
                    lt_temp[:, 1] == self.linked_triangles[i, 1]))  # rightward
...
class dynamic_points_analysis_2d:  # old_class_name: msd
    ...
    def compute_nearest_neighbor_displacements(self, csv_prefix='', bond_cut_off=6):
        ...
        for frame in list_frames:
            ...
            result.get_first_minima_bond_length_distribution(
                lattice_constant=1, hist_cutoff=bond_cut_off)
    ...
    def monitor_neighbor_change_event(self, ts_id_dxy, csv_prefix=''):
        ...
        list_id_nb_not_cut_edge = list_id_counts[0, list_id_counts[1, :] == max(
            list_id_counts[1])]  # list_id_counts[1].max]
        n_frame_nb_stable = max(list_id_counts[1])
        n_particle_nb_stable = len(list_id_nb_not_cut_edge)

        list_sum_id_nb_stable = list_sum_id_nb[list_sum_id_nb["particle_id"].isin(
            list_id_nb_not_cut_edge)]
        list_sum_id_nb_stable_np = list_sum_id_nb_stable["sum_id_neighbors"].values
        list_sum_id_nb_stable_np = np.reshape(
            list_sum_id_nb_stable_np, (n_frame_nb_stable, n_particle_nb_stable))
        ...
        list_sum_id_nb_stable.loc[:, 'if_nb_change'] = tuple(
            np.reshape(if_nb_change, n_frame_nb_stable*n_particle_nb_stable))
    ...
    def plot_bond_neighbor_change(self, data_name='default_exp', prefix='',
        ...
        for i in range(num_of_frames):
            ...
            if final_cut:
                ...
                png_filename2 = prefix + 'bond_plot_1st_minima_index' + \
            ...
            a_frame.draw_bonds_conditional_bond(
                ...
                png_filename=png_filename2, show_traps=show_traps, LinearCompressionRatio=trap_lcr,
                trap_filename=trap_filename, nb_change=ids, x_unit=self.x_unit)
    ...
    def plot_bond_neighbor_change_oop(self, data_name='default_exp', prefix='',
        ...
        for i in range(num_of_frames):
            ...
            a_frame.draw_bonds_conditional_bond_oop(
                check=[0.4, a_frame.bond_first_minima_left],
                png_filename=png_filename2, xy_stable=self.txyz_stable[i],
                nb_change=ids, x_unit=self.x_unit, LinearCompressionRatio=trap_lcr,
    ...
    def plot_string_like_motion(self, frame_init, frame_final, prefix='',
        ...
        if ids is None:
            ...
            bpm.ax.scatter(
                ...
                xy_final[ids, 1],
                facecolors='none', edgecolors=circle_color, marker='o', s=circle_size,
                linewidths=lw, zorder=2)  # circle, facecolors='none',fillstyle='none'[x]
        # draw traps
        if not trap_filename is None:
            bpm.plot_traps(trap_filename, trap_lcr, 'map')

        # save figure
        png_filename2 = prefix + 'string_like_motion'+str_index+'_'+str(
            int(frame_init))+'_'+str(int(frame_final))+'.png'  # '.pdf'
        bpm.save_figure(png_filename2)
        return ids_in
    ...
    def plot_string_like_motion_rank(self, frame_init, frame_final, prefix='',
        ...
        # save figure
        png_filename2 = prefix + 'string_like_motion_rank'+str_index+'_' + \
            str(int(frame_init))+'_'+str(int(frame_final))+'.png'  # '.pdf'
        bpm.save_figure(png_filename2)
        return ids_active

    def neighbor_change_id_method(self, nb_change, frame_init, frame_final, ids_active):
        list_sum_id_nb_stable = nb_change
        list_frames = (list_sum_id_nb_stable['frame'] >= frame_init) & (
            list_sum_id_nb_stable['frame'] <= frame_final)
    ...
    def get_saturate_time_of_tanh_like_data(self, orderp, threshold=0.95):
        ...
        parameters:
            ...
            threshold: the threshold of the order parameter, which is used to define the saturation time.
        return:
            saturate_time: the time when the order parameter reaches threshold
            averaged_velocity: the averaged velocity of the order parameter from activation to saturation.
        """
        ...
        for i in range(1, np.shape(data_decorated)[0]):
            # compared with orderp[-1], data_decorated[-1] is better
            # as a benchmark of the dynamic ending
            if data_decorated[-1] == 0:
                saturate_index = np.shape(data_decorated)[0]-1
                break
            if data_decorated[i]/data_decorated[-1] >= threshold:
                saturate_index = i
                break
        """isExist = "saturate_index" in locals()
        if not isExist:
            saturate_index = np.shape(data_decorated)[0]-1"""
        saturate_time = list_index[saturate_index]
        averaged_velocity = data_decorated[saturate_index]/saturate_time
        return saturate_time, averaged_velocity
...
class trajectory_module:
    ...
    def trajectory_coarse_grain_general(self, txyz_stable_id=16, n_frame_to_coarse=2, tchip=None,
        ...
        if mode == "trajectory_displacement":
            # '/home/tplab/xiaotian_file/lxt_code_py/4302_9/
            file_t_pin_bool = prefix+'pin_check/t_pin_bool.npy'
        ...
        if txyz_stable_id is None:  # proceed trajectories for all particles
            ...
            if mode == "trajectory_displacement":
                ...
                # to check if the particle(id) truly jumps
                list_if_jump = np.ones((sz[1],)).astype(bool)
            if ax is None:
                fig, ax = plt.subplots()
            for id in range(sz[1]):
                ...
                if mode == "trajectory_displacement":
                    ...
                    if count_jump[id] < 2:  # to check if the particle(id) truly jumps
                        ...
                        record_last_jump_xy[id] = [
                            txyz_stable[-1, id, 0],
                            txyz_stable[-1, id, 1],
                            txyz_stable[-1, id, 0],
                            txyz_stable[-1, id, 1]]  # to show static particles
                    else:
                        last_jump_xy = self.plot_bicolor_trajectory(
                            txyz_stable
                            [list_jump_bool[:, id],
                             id, : 2],
                            t_pin_bool[fend, id],
                            ax=ax, width=linewidth,
                            color_pin=cp, color_inte=ci)
            ...
            # txyz_stable[-1,:]-record_last_jump_xy
            uv = record_last_jump_xy[:, 2:]-record_last_jump_xy[:, :2]
            lim = [[-18, 18], [-18, 18]]
            self.plot_bicolor_final_displacements(
                uv, record_last_jump_xy[:, 0: 2],
                t_pin_bool[fend],
                list_if_jump, ax, limit=lim, width=width, color_pin=cp, color_inte=ci)  # txyz_stable[-1,:]
            self.plot_bicolor_final_displacements_static(
                record_last_jump_xy[:, 0: 2],
                t_pin_bool[fend],
                list_if_static, ax, limit=lim, size=size, color_pin=cp, color_inte=ci)
            # plt.show()
    ...
    # ,hist_cutoff=2,png_filename=None,x_unit='(1)'
    def get_first_minima_count_of_distribution(self, method="global_compare_method"):
        R"""  
        Introduction:
        ...
        # plt.figure()
        # plt.hist(self.displacement_length,bins=20,range=[0,hist_cutoff])
        ...
        self.displacement_first_neighbour = self.displacement_sorted[np.where(
            self.displacement_sorted[:] < self.displacement_first_minima_left)]
    ...
    # ,list_jump_bool
    def plot_bicolor_trajectory(
            self, txy, t_pin_bool, ax=None, width=1, color_pin='r', color_inte='orange'):
        R"""
        input:
    ...
    def plot_bicolor_final_displacements(self, uv, xy, list_pin_true, list_if_jump, ax=None, x_unit='($\sigma$)', limit=None, width=0.01, color_pin='r', color_inte='orange'):  # ,png_filename=None
        ...
        if not limit is None:
            ...
            ax.set_ylim(limit[1])
            ax.quiver(
                xy[list_pin_true_1, 0],
                xy[list_pin_true_1, 1],
                uv[list_pin_true_1, 0],
                uv[list_pin_true_1, 1],
                color=color_pin, angles='xy', scale_units='xy', scale=1, width=width)  # ,width=0.01,color='r'
            ax.quiver(
                xy[list_pin_false_1, 0],
                xy[list_pin_false_1, 1],
                uv[list_pin_false_1, 0],
                uv[list_pin_false_1, 1],
                color=color_inte, angles='xy', scale_units='xy', scale=1, width=width)
            ...
            ax.quiver(
                ...
                uv[list_pin_true_1, 0],
                uv[list_pin_true_1, 1],
                color=color_pin, angles='xy', scale_units='xy', scale=1, width=width)  # ,color='r'
            ax.quiver(
                xy[list_pin_false_1, 0],
                xy[list_pin_false_1, 1],
                uv[list_pin_false_1, 0],
    ...
    # ,png_filename=None
    def plot_bicolor_final_displacements_static(
            self, xy, list_pin_true, list_if_static, ax=None, limit=None, size=1, color_pin='r',
            color_inte='orange'):
        list_pin_false = np.logical_not(list_pin_true)
...
class mean_square_displacement:
    ...
    def compute_atmsd_t_chips(self, interval_max=0.1, msd_per_particle=False):
        ...
        for m in list_m:
            # how many chips the time axis is divided into
            chips = int((self.txyz_stable.shape[0]-1)/m)
    ...
    def compute_nn_msd_X(self, unit='sigma'):
        ...
        for frame in list_frames:
            ...
            if unit == 'sigma':
                ...
                lc = 2.0
            # here lattice_constant is just used to normalize figure, hence set 2.0 is ok
            result.get_first_minima_bond_length_distribution(lattice_constant=lc, hist_cutoff=5)
            # print('recognized bond length: '+str(result.bond_length_median*lc)+'+-'+str(result.bond_length_std*lc)+' '+unit)
            list_1st_bond_length_framewise[frame] = result.bond_length_median  # unit = sigma!
            # displacement neighbor
    ...
    def plot_msd_t_chips(
            self, time_log=None, png_filename='msd_chips_long_loglog.png', sigma=False, um_sec=True,
            lindemann=False):
        R"""
        introduction:
    ...
    def plot_lindemann_msd(
            self, m_msd, average_1st_bond_length, time_log=None,
            png_filename='lindemann_msds_loglog.png'):
        R"""
        input: 
...
class displacemnt_field_2D:
    ...
    def get_displacement_field_xy(
            self, frame_index_start=0, frame_index_end=-1, plot=False, png_filename=None,
            x_unit='($\sigma$)', limit=None):
        R"""
        Introduction:
        ...
        if plot:
            if not limit is None:
                ...
                self.ax.quiver(xy[:, 0], xy[:, 1], uv[:, 0], uv[:, 1], color='orange',
    ...
    def get_bicolor_disp(
            self, list_pin_bool, frame_index_start=0, frame_index_end=-1, plot=False,
            png_filename=None, x_unit='($\sigma$)', limit=None, traps=None):
        R"""
        Introduction:
            The function draws a displacement vector field from init state to final state 
            with positions at edge removed  to clean abnormal displacement vector. 
        ...
        if plot:
            if not limit is None:
                ...
                self.ax.quiver(
                    ...
                    uv[list_pin_nbool, 0],
                    uv[list_pin_nbool, 1],
                    color='orange', angles='xy', scale_units='xy', scale=1, width=0.01)
            # self.ax.scatter(self.final_positions[:,0],self.final_positions[:,1])#final_state
            else:
                ...
                self.ax.quiver(
                    ...
                    uv[list_pin_bool, 0],
                    uv[list_pin_bool, 1],
                    color='r', angles='xy', scale_units='xy', scale=1)  # ,width=0.01,color='r'
                self.ax.quiver(
                    xy[list_pin_nbool, 0],
                    xy[list_pin_nbool, 1],
                    uv[list_pin_nbool, 0],
    ...
    def get_displacement_field_xy_id(
            self, frame_index_start=0, frame_index_end=-1, x_unit='($\sigma$)', color='limegreen'):
        R"""
        Introduction:
            The function draws a displacement vector field from init state to final state 
            with positions at edge removed  to clean abnormal displacement vector. 
        ...
        # self.ax.scatter(self.final_positions[:,0],self.final_positions[:,1])#final_state
        self.ax.quiver(xy[ids, 0], xy[ids, 1], uv[:, 0], uv[:, 1],
    ...
    # ,color='limegreen'
    def get_displacement_field_xy_rank(
            self, rank, frame_index_start=0, frame_index_end=-1, x_unit='($\sigma$)'):
        R"""
        Introduction:
            The function draws a displacement vector field from init state to final state 
            with positions at edge removed  to clean abnormal displacement vector. 
        ...
        mapp = self.ax.quiver(xy[ids, 0], xy[ids, 1], uv[:, 0], uv[:, 1],
                              rank, angles='xy', scale_units='xy', scale=1)  # ,rank
...
class bond_plot_module_for_image:
            ...
            for i in range(np.shape(bond_length)[0]):
                if (bond_length[i, 2] > bond_check[0]) & (bond_length[i, 2] < bond_check[1]):
                    ...
                    line = plt.Polygon([pt1, pt2], closed=None, fill=None,
                                       edgecolor='b', zorder=0, lw=0.5)  # ,lineStyle='dashed'
                    self.ax.add_line(line)
            self.ax.set_title("bond_length:"+str(np.around(bond_length_limmit, 2)
                                                 )+self.x_unit)  # Add a title to the axes
    ...
    def plot_traps(
            self, trap_filename="/home/tplab/hoomd-examples_0/testhoneycomb3-8-12-part1",
            LinearCompressionRatio=0.79, mode='array'):
        R"""
        trap_filename:
                '/home/remote/hoomd-examples_0/testhoneycomb3-8-12'
                '/home/remote/hoomd-examples_0/testhoneycomb3-8-12-part1'
    ...
    # compatible module
    def draw_bonds_conditional_bond(
            self, xy, bond_length, bond_length_limmit=[0.9, 2.0],
            x_unit='(um)'):
        R"""
        Parameters:
            txyz: all the particle positions, no one removed.
        ...
        self.ax.set_title("bond_length:"+str(np.around(bond_length_limmit, 2))+x_unit)
        """
        the displayed image size will be the smaller one 
        between axis limitation for xlim/ylim or data itself. 
        Hence the effective xlim/ylim should be set smaller than data.
...
class dynamical_facilitation_module:
    ...
    # ,png_filename=None
    def get_facilitation_waiting_time_distributions_1(
            self, reference_occupation, prefix=None, io_only=False):
        R"""
        input:
    ...
    # [x]
    def get_facilitation_waiting_time_distributions_0(
            self, search_nb, reference_occupation, prefix=None):
        R"""
    ...
    # [x]
    def cut_facilitation_waiting_time(self, list_waiting_time_frame_id, wait_max, prefix=None):
        R"""
        flopping simultaneously (coarse grained step) is counted in waiting time
        """
    ...
    def get_interstitial_bool(self, reference_positions, txyz, result_prefix=None, r_trap=1):
        ...
        list_interstitial_bool = self.get_pin_bool(
            dual_pt, txyz, result_prefix, r_trap, ispin=False)
    ...
    def plot_reference_occupation(self, reference_positions, reference_occupation, prefix=None):
        ...
        for frame in range(sz[0]):
            ...
            lw = 2
            ax.scatter(
                reference_positions[vac, 0],
                reference_positions[vac, 1],
                facecolors='none', edgecolors='k', marker='o', s=circle_size, linewidths=lw)
            ax.scatter(
                reference_positions[occu, 0],
                reference_positions[occu, 1],
                c='k', marker='o', s=circle_size)
    ...
    def plot_reference_positions_waiting_time_colored(
            self, reference_positions, reference_occupation, prefix=None):
        R"""
        EXAMPLE:
            #get box from gsd
            gsd_filename=None#"/home/remote/hoomd-examples_0/trajectory_auto4302_9.gsd"
...
class polygon_analyzer_ml:
    ...
    # [x]
    def scan_conditional_bonds_and_simplices_ml(
            self, scan_mode='local_scan', png_filename=None, cost_mode='mean_var'):
        R"""
        input:
        ----------
            mode:
        ...
        scan_ridge_length_rank_min = self.check_scan_ridge_length_rank(
            scan_ridge_length_rank_min, n_ridges)
        scan_ridge_length_rank_max = (ridge_length_rank_ratio + scan_range_relative)*n_ridges
        scan_ridge_length_rank_max = self.check_scan_ridge_length_rank(
            scan_ridge_length_rank_max, n_ridges)
        list_short_ridge_rank = np.linspace(
            scan_ridge_length_rank_min, scan_ridge_length_rank_max - 1, scan_ridge_length_rank_max -
            scan_ridge_length_rank_min)
        ...
        print('ridge_length = ', np.round(
            ridge_length_sorted[ridge_rank_min_cost], 3), ' is best\n')

        if not png_filename is None:
            fig, ax = plt.subplots()
            ax.plot(ridge_length_sorted, record_costs[:, 0], 'r', label='cost')  # ,legend
            ax.plot(ridge_length_sorted, record_costs[:, 1],
                    'g', label='$r(n_{cluster})$')  # n_cluster_norm
            if cost_mode == 'sum_var':
                ax.plot(
                    ridge_length_sorted, record_costs[:, 2],
                    'b', label='$r(\Sigma \sigma^2)$')  # sum_var_norm
            elif cost_mode == 'mean_var':
                ax.plot(
                    ridge_length_sorted, record_costs[:, 2],
                    'b', label='$r(\langle \sigma^2 \rangle)$')  # mean_var_norm
                # '$r(\overline{\sigma^2})$' looks ugly
            ax.scatter(ridge_length_sorted[short_ridge_length_rank], record_costs[int(
                short_ridge_length_rank-list_short_ridge_rank[0]), 0], c='k')
    ...
    def get_conditional_bonds_and_simplices_ml(self, ridge_first_minima_ml):  # [x]
        ...
        list_vertices_index = np.unique(list_ridge_vertices_index)
        # count the vertices of short ridge to form cluster
        n_vertices_in_cluster = np.shape(list_vertices_index)[0]
        ...
        # watch initial value, 0,1,2...
        record_vertices_cluster[:, 1] = range(n_vertices_in_cluster)
        record_vertices_cluster[:, 0] = list_vertices_index
        # cluster_id = 0
        ...
        # n_polygon also
        n_cluters_and_vertex = n_vertices_reasonable - n_vertices_in_cluster + len(list_cluster_id)
        check_1 = False
        if check_1:
            print('n_cluters_and_vertex', 'n_polygon')
            print(n_cluters_and_vertex, sum(count_polygon[:, 1]))
            # np.shape(list_short_bonds)[0]
            np.sum(np.logical_not(list_short_ridge_bool).astype(int))
        # print("polygon_n, count\n",count_polygon)
        count_polygon_relative = np.array(count_polygon)
        ...
        # self.vertex_bonds_index = list_short_bonds#short bonds index offered by ridge comapre method
        # n_vertices rows of [vertex_id, cluster_id],
        self.list_simplex_cluster = record_vertices_cluster
    ...
    # ,list_cluster_ids,vertex_cluster_positions[x]
    def get_cost_function_cluster_ml_method_sum_var(self):
        R"""
        introduction:
        ...
        # this is too large but okay anyway.
        list_var_of_clusters = np.zeros((len(list_cluster_ids_unique),))
        ...
        for i in range(len(list_cluster_ids_unique)):
            ...
            list_vertices_indices = np.array(
                self.list_simplex_cluster[list_vertices_indices_bool, 0]).astype(int)
            n_vertices_in_cluster_id = len(list_vertices_indices)  # np.sum .astype(int)
            list_var_of_clusters[i] = np.var(
                vertex_positions[list_vertices_indices, 0])+np.var(vertex_positions[list_vertices_indices, 1])
            # consider the contribution of each vertex
            list_var_of_clusters[i] = list_var_of_clusters[i] * n_vertices_in_cluster_id
...
class energy_computer:
    ...
    def compute_energy(self, extended_positions):
        ...
        """
        list_interaction_distance = self.compute_distance(
            extended_positions, self.r_cut_interaction, True)
        ...
        list_energy_interaction_step2 = np.divide(
            list_energy_interaction_step1, 2*list_interaction_distance)
        # the 2 at denominator are used to calculate particle-wise energy only once, no overlaps.
        list_energy_pin = 0.5*self.k*(np.square(list_trap_distance)
                                      - self.r_cut_trap*self.r_cut_trap)
    ...
    def compute_energy_particle_wise(self, extended_positions):  # [ongoing]
        ...
        """
        list_interaction_distance = self.compute_distance(
            extended_positions, self.r_cut_interaction, True, True)
        ...
        for j in range(n_particles):
            list_energy_interaction_step1 = self.a_yukawa * \
                np.exp(-self.kappa*list_interaction_distance[j])
            list_energy_interaction_step2 = np.divide(
                list_energy_interaction_step1, 2*list_interaction_distance[j])
            # the 2 at denominator are used to calculate particle-wise energy only once, no overlaps.
            list_energy_pin = 0.5 * self.k * (
                np.square(list_trap_distance[j]) - self.r_cut_trap * self.r_cut_trap)
            ...
            self.energy_inter_pin_mecha[j, 2] = self.energy_inter_pin_mecha[j,
                                                                            0] + self.energy_inter_pin_mecha[j, 1]

    def compute_distance(self, reference_positions, r_cut, interact_mode=False,
                         particle_wise_mode=False):
        R"""
        Introduction:
        ...
        if interact_mode:
            ...
            check_if_effective_interaction = np.logical_and(
                check_if_effective_interaction, check_if_effective_interaction2)
    ...
    def get_distance_as_array(self, check_if_effective_interaction):
        for i in range(len(check_if_effective_interaction)):
            # if it is true?
            list_dis_value_one_row = self.list_interparticle_distance[i,
                                                                      check_if_effective_interaction[i]]
            # print(list_dis)
            if i == 0:
