...
class simulation_controller_honeycomb_traps:
    ...
    def generate_initial_state_hexagonal_particle_honeycomb_trap_single(self):
        ...
        isg.set_new_gsd_file_2types_by_n_size(
            particles, n_size, particle_points, traps, gsd_filename)
        ...
        dac.save_from_gsd(
            9999, 9, bond_plot=True, show_traps=True, trap_filename=save_file_txt, trap_lcr=1.0,
            list_traps=list_t, account='remote')
    ...
    def generate_initial_state_hexagonal_particle_honeycomb_trap_scan_k(self):
        ...
        isg.set_new_gsd_file_2types_by_n_size(
            particles, n_size, particle_points, traps, init_gsd_filename)
    ...
    def generate_simu_index_csv_6373(self):
        ...
        n_gauss_epsilons = 30
        list_gauss_epsilon = np.linspace(
            gauss_epsilon1, gauss_epsilon1 + gauss_epsilon_step * (n_gauss_epsilons - 1),
            n_gauss_epsilons)
    ...
    def generate_simu_index_csv_5773_5812(self):
        ...
        gauss_epsilon_step = -3
        n_gauss_epsilons = 20
        list_gauss_epsilon1 = np.linspace(
            gauss_epsilon1, gauss_epsilon1 + gauss_epsilon_step * (n_gauss_epsilons - 1),
            n_gauss_epsilons)
        gauss_epsilon1 = -100
        gauss_epsilon_step = -100
        n_gauss_epsilons = 10
        list_gauss_epsilon2 = np.linspace(
            gauss_epsilon1, gauss_epsilon1 + gauss_epsilon_step * (n_gauss_epsilons - 1),
            n_gauss_epsilons)
        gauss_epsilon1 = -63
        gauss_epsilon_step = -3
        n_gauss_epsilons = 10
        list_gauss_epsilon3 = np.linspace(
            gauss_epsilon1, gauss_epsilon1 + gauss_epsilon_step * (n_gauss_epsilons - 1),
            n_gauss_epsilons)
        list_gauss_epsilon = np.concatenate(
            (list_gauss_epsilon1, list_gauss_epsilon2, list_gauss_epsilon3))
        list_lcr = 0.81
        temperature = 1
        t = temperature
        ...
        for seed in range(10):
            ...
            for gauss_epsilon in list_gauss_epsilon:
                if ((simu_index == index1) & (seed == 0)):
                    ...
                    record1 = pd.DataFrame(
                        [[simu_index, seed, list_lcr, gauss_epsilon, t]],
                        columns=column)
                    record_all = pd.concat([record_all, record1])
                    simu_index = simu_index + 1
        pd.DataFrame.to_csv(record_all, output_file_csv)
    ...
    def generate_initial_state_hexagonal_particle_honeycomb_trap_scan_csv(self, csv_filename):
        ...
        for i in range(len(list_simu_index)):
            ...
            init_gsd_filename = generate_initial_gsd_hex_and_type_n(
                3, a_particle, n_size, lcr)  # self.generate_initial_gsd(a_particle,n_size,lcr)
    ...
    def generate_initial_gsd(sel, a_particle, n_size, lcr):  # deprecated
        ...
        filename_initial_state = 'init_particle_hex_'+str(a_particle)+'_'+str(
            n_size[0])+'_'+str(n_size[1])+'_and_trap_honeycomb_'+str_lcr
        ...
        if isExists:
            ...
            isg.set_new_gsd_file_2types_by_box_or_n_size(
                particles, traps, init_gsd_filename, perturb=False, n_size=n_size)
...
class simulation_controller_honeycomb_part_traps:
    ...
    def generate_simu_index_csv_lcr081(self):
        ...
        output_file_csv = prefix_write + \
            'pin_hex_to_honeycomb_part_klt_2m_gauss_'+str(int(index1))+'.csv'
        # list_simu_index = np.linspace(index1,index1+n_files-1,n_files,dtype=int)
        gauss_epsilon1 = -3
        gauss_epsilon_step = -3
        n_gauss_epsilons = 30
    ...
    def generate_initial_state_hexagonal_particle_honeycomb_part_gaus_eq_harmo_trap_scan_csv(
            self, csv_filename):
        R"""
        Introduction:
                a comprehensive workflow from creating initial state, 
    ...
    def generate_initial_gsd(sel, a_particle, n_size, lcr):
        ...
        str_lcr = str(lcr)  # '081'#in case the string like '0.810000000032'
        filename_initial_state = 'init_particle_hex_'+str(a_particle)+'_'+str(
            n_size[0])+'_'+str(n_size[1])+'_and_trap_honeycomb_part_'+str_lcr
...
class simulation_controller_type_n_part_traps:
    ...
    def generate_simu_index_csv_depin(self):  # not finished
        ...
        # 5933,
        output_file_csv = prefix_write + \
            'depin_type_n_from_type_n_part_klt_2m_gauss_'+str(int(index1))+'.csv'
        # list_simu_index = np.linspace(index1,index1+n_files-1,n_files,dtype=int)
        gauss_epsilon1 = -3
        gauss_epsilon_step = -3
        n_gauss_epsilons = 30
        ...
        for i in range(list_type_n.shape[0]):  # type and lcr
            for gauss_epsilon in list_gauss_epsilon:
                if (simu_index == index1):
                    record1 = [
                        [index1, seed, list_lcr[0],
                         list_gauss_epsilon[0],
                         t, list_type_n[0]]]
                    column = ['simu_index', 'seed', 'lcr',
                              'trap_gauss_epsilon', 'temperature', 'type_n']  # ,'cn3'
                    record_all = pd.DataFrame(record1)
                    record_all.columns = column
                    simu_index = simu_index + 1
                else:
                    record1 = pd.DataFrame(
                        [[simu_index, seed, list_lcr[i],
                          gauss_epsilon, t, list_type_n[i]]],
                        columns=column)
                    record_all = pd.concat([record_all, record1])
                    simu_index = simu_index + 1
        pd.DataFrame.to_csv(record_all, output_file_csv)
    ...
    def generate_simu_index_csv_depin_special_low_t(self):  # not finished
        ...
        # 5933,
        output_file_csv = prefix_write + \
            'depin_type_n_from_type_n_part_special_klt_2m_gauss_'+str(int(index1))+'.csv'
        # list_simu_index = np.linspace(index1,index1+n_files-1,n_files,dtype=int)
        gauss_epsilon1 = -3
        gauss_epsilon_step = -3
        n_gauss_epsilons = 10
    ...
    def generate_simu_index_csv_pin_3_30(self):  # not finished
        ...
        # 5933,
        output_file_csv = prefix_write + \
            'pin_hex_to_type_n_part_klt_2m_gauss_'+str(int(index1))+'.csv'
        # list_simu_index = np.linspace(index1,index1+n_files-1,n_files,dtype=int)
    ...
    def generate_initial_state_type_n_particle_type_n_part_trap_depin_scan_csv(self, csv_filename):
        ...
        for i in range(len(list_simu_index)):
            if i >= 0:
                ...
                gauss_epsilon = list_trap_gauss_epsilon[i]
                temperature = list_temperature[i]
                init_gsd_filename = self.generate_initial_gsd_type_n_and_part(
                    list_type_n[i], a_particle, box, lcr)
                print(simu_index, seed, lcr, gauss_epsilon, temperature, init_gsd_filename)
                sim = sco.simulation_core_traps(simu_index, seed)
                sim.seed = seed
    ...
    def generate_initial_state_hex_particle_type_n_part_trap_scan_csv(self, csv_filename):
        ...
        for i in range(len(list_simu_index)):
            ...
            temperature = list_temperature[i]
            init_gsd_filename = generate_initial_gsd_hex_and_type_n(
                list_type_n[i], a_particle, n_size, lcr, True)
    ...
    def generate_initial_gsd_type_n_and_part(self, type_n, a_particle, box, lcr):
        ...
        # to prevent the mixing of different initial state .gsd file
        str_lcr = str(lcr)  # '081'#in case the string like '0.810000000032'
        filename_initial_state = 'init_particle_type_' + str(type_n) + '_' + str(a_particle) + '_' + str(
            box[0]) + '_' + str(box[1]) + '_and_trap_type_' + str(type_n) + '_part_' + str_lcr
        ...
        if isExists:
            ...
            isg = pg.initial_state_generator()
            isg.set_new_gsd_file_2types_by_box_or_n_size(
                particles, traps, init_gsd_filename, box=box, perturb=True)  # ,perturb=True
...
class simulation_controller_kagome_traps:
            ...
            for lcr in list_lcr:
                for gauss_epsilon in list_gauss_epsilon:
                    if ((simu_index == index1) & (seed == 0)):
                        ...
                        record1 = pd.DataFrame(
                            [[simu_index, seed, lcr, gauss_epsilon, t, list_type_n]],
                            columns=column)
                        record_all = pd.concat([record_all, record1])
                    simu_index = simu_index + 1
    ...
    def generate_initial_state_hex_particle_kagome_trap_scan_csv(self, csv_filename):
        ...
        for i in range(len(list_simu_index)):
            ...
            temperature = list_temperature[i]
            init_gsd_filename = generate_initial_gsd_hex_and_type_n(
                list_type_n[i], a_particle, n_size, lcr)
            # init_gsd_filename = self.generate_initial_gsd_hex_and_type_n_part(list_type_n[i],a_particle,n_size,lcr)
...
class simulation_controller_kagome_part_traps:
    ...
    def generate_initial_state_hex_particle_kagome_part_trap_scan_csv(self, csv_filename):
        ...
        for i in range(len(list_simu_index)):
            ...
            temperature = list_temperature[i]
            init_gsd_filename = generate_initial_gsd_hex_and_type_n(
                list_type_n[i], a_particle, n_size, lcr, type_n_part=True)
            # init_gsd_filename = self.generate_initial_gsd_hex_and_type_n_part(list_type_n[i],a_particle,n_size,lcr)
            sim = sco.simulation_core_traps(simu_index, seed)
            sim.gauss_sigma = 0.6
            sim.gauss_r_cut = 2.0
...
def generate_initial_gsd_hex_and_type_n(type_n, a_particle, n_size, lcr, type_n_part=False):
    ...
    if type_n_part:
        filename_initial_state = 'init_particle_hex_'+str(a_particle)+'_'+str(
            n_size[0])+'_'+str(n_size[1])+'_and_trap_type_'+str(type_n)+'_part_'+str_lcr
    else:
        filename_initial_state = 'init_particle_hex_'+str(a_particle)+'_'+str(
            n_size[0])+'_'+str(n_size[1])+'_and_trap_type_'+str(type_n)+'_'+str_lcr
    init_gsd_filename = prefix+filename_initial_state+'.gsd'  # describe the lcr
    ...
    if isExists:
        ...
        isg = pg.initial_state_generator()
        isg.set_new_gsd_file_2types_by_box_or_n_size(
            particles, traps, init_gsd_filename, perturb=False, n_size=n_size)  # False
        """#check the initial configuration of points and traps
        #isg = pg.initial_state_generator()
        isg.read_gsd_file( init_gsd_filename)
        points = isg.particles.position
